import NewKecermatanModel from "../../../Model/Question/New_Kecermatan";
...
class Kecermatan implements KecermatanService {
    ...
    public async findAll(secureId: string, type: string): Promise<any> {
        try {
            const [KecermatanDetail] = await this.newKecermatanGroupModel.findOne(secureId);
            if (!KecermatanDetail) throw "Group Not Found";

            const Kecermatan = await this.newKecermatanModel.findAll(secureId);
    ...
    public async insertSection(payload: PayloadCreateKecermatanSectionVO): Promise<any> {
        try {
            const [Group] = await this.newKecermatanModel.findOneGroup(payload.groupSecureId);
            ...
            if (!payload.secureId) {
                ...
                const Create = await this.newKecermatanModel.createSection(Payload)
                if (!Create) throw "Create Section Error";

            } else {
                Payload.secureId = payload.secureId
                const Update = await this.newKecermatanModel.updateSection(Payload)
    ...
    public async insertQuestion(payload: PayloadRequestKecermatanQuestionVO): Promise<any> {
        try {
            const [Section] = await this.newKecermatanModel.findOneSection(payload.sectionSecureId);
            ...
            if (!payload.secureId) {
                ...
                Question = await this.newKecermatanModel.createQuestion(payload_insert);
                if (!Question) throw "Create Question Error";
            } else {
                isUpdate = true;
                [FindQuestion] = await this.newKecermatanModel.findOneQuestion(payload.secureId);
                ...
                Question = await this.newKecermatanModel.updateQuestion(payload_insert);
            ...
            if (payload.answerList.every(e => e.secureId)) {
                const FindAnswer = await this.newKecermatanModel.findAllAnswer(payload_insert.secureId);
                if (FindAnswer.length != 0) {
                    const Delete = await this.newKecermatanModel.deleteAnswer(FindAnswer);
            ...
            const Answer = await this.newKecermatanModel.createAnswer(PayloadAnswer)
    ...
    public async deleteSection(secureId: string): Promise<any> {
        try {
            const Question = await this.newKecermatanModel.deleteSection(secureId);
    ...
    public async deleteQuestion(secureId: string): Promise<any> {
        try {
            const Question = await this.newKecermatanModel.deleteQuestion(secureId);
            if (!Question) throw "Delete Data Error";
